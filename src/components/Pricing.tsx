...
const Pricing = () => {
  ...
  return <section id="pricing" className="bg-white py-24">
      <div className="container-custom">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {plans.map((plan, index) => <div key={plan.name} className={cn("rounded-2xl relative animate-on-scroll transition-all duration-500 foam-bubbles", "transform hover:-translate-y-2", plan.popular ? "bg-gradient-to-br from-blue/5 to-blue/20 shadow-[0_8px_30px_rgba(30,136,229,0.15)]" : "bg-white shadow-[0_0_15px_rgba(0,0,0,0.05)]")} ref={el => elementsRef.current[index + 2] = el} style={{
          animationDelay: `${index * 200}ms`
        }}>
              {plan.popular && <div className="absolute -top-4 -right-4 bg-gradient-to-r from-blue to-blue-dark text-white px-6 py-2 rounded-lg shadow-lg text-sm font-medium">
                  Populær
                </div>}
              ...
              <div className="p-8">
                ...
                <a href="#contact" className={cn("relative overflow-hidden rounded-md w-full py-3 px-6 font-semibold transition-all duration-300 text-center block", "hover:shadow-lg hover:scale-105 active:scale-95", plan.popular ? "bg-blue text-white shadow-md" : "bg-gray-100 text-gray-800 hover:bg-gray-200")}>
                  <span className="relative z-10">Kom i Gang</span>
                  {plan.popular && <span className="absolute inset-0 bg-blue-dark transform scale-x-0 origin-left transition-transform duration-300 ease-out group-hover:scale-x-100"></span>}
                </a>
              </div>
            </div>)}
        </div>
