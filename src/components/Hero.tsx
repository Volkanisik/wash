...
const Hero = () => {
  ...
  return <section id="home" className="min-h-screen flex items-center pt-20 overflow-hidden">
      <div className="container-custom grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
        ...
        <div className="order-1 lg:order-2 relative">
          <div className={cn("relative rounded-2xl overflow-hidden shadow-2xl opacity-0 transform translate-y-8 foam-bubbles", isVisible && "animate-fade-in opacity-100 translate-y-0 transition-all duration-1000 delay-300")} ref={el => elementsRef.current[4] = el}>
            <img src={imageError ? fallbackImage : heroImage} alt="Professionel bilvask i aktion" className="w-full h-auto rounded-2xl" onLoad={handleImageLoad} onError={handleImageError} />
            <div className="absolute inset-0 bg-gradient-to-t from-black/30 to-transparent"></div>
          </div>
          
          <GlassPanel className={cn("absolute -bottom-5 -left-5 md:bottom-8 md:left-8 p-4 max-w-[200px] z-10", "opacity-0 transform translate-y-8", isVisible && "animate-fade-in opacity-100 translate-y-0 transition-all duration-1000 delay-800")} ref={el => elementsRef.current[5] = el} glowColor="rgba(74, 222, 128, 0.3)" glowIntensity="medium">
            <div className="text-green font-bold text-lg">15 min</div>
            <p className="text-sm text-gray-600">Gennemsnitlig ventetid</p>
          </GlassPanel>
          
          <GlassPanel className={cn("absolute -top-5 -right-5 md:top-8 md:right-8 p-4 z-10", "opacity-0 transform translate-y-8", isVisible && "animate-fade-in opacity-100 translate-y-0 transition-all duration-1000 delay-1000")} ref={el => elementsRef.current[6] = el} glowColor="rgba(59, 130, 246, 0.3)" glowIntensity="medium">
